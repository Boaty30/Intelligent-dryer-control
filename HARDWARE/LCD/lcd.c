...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum_HEX(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode)
{  
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
	 	 ...
	 	 if (temp<=9)
	 	    ...
	 	    LCD_ShowChar(x+(size/2)*t,y,temp+'A'-10,size,mode&0X01);  
	}
} 
void LCD_ShowxNum_DEC(u16 x,u16 y,u32 num,u8 len,u8 size,u8 mode)
{  
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
